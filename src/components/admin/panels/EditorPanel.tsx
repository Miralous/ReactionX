...
export default function EditorPanel() {
  const {
    ...
    currentMode, filename, setFilename,
    stageForWrite, isFetchingContent, currentCollection
  } = useAdmin();
  ...
  const isFriend = currentMode === 'friend';
    ...
    <div className={cn(
        ...
        <div className="flex items-center h-full flex-1 min-w-0">
          ...
          <div className="flex-1 flex items-center gap-2 max-w-sm">
            <span className={cn(
              ...
              isFriend ? "icon-[ph--link]" : isDynamic ? "icon-[ph--waveform]" : "icon-[ph--file-text]"
            )}></span>
            <input
              value={filename}
      ...
      <div className="flex-1 relative w-full h-full min-h-[400px] flex flex-col overflow-hidden bg-background">
        ...
        {isPost ? (
          ...
          <DynamicEditor showPreview={showPreview} />
        ) : isFriend ? (
          <FriendEditor />
        ) : null}
      </div>
