...
export function useAdminEditor(showToast: (msg: string, type?: ToastType) => void) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        const uploadUrl = new URL(import.meta.env.PUBLIC_IMG_BED_URL || '');
        uploadUrl.searchParams.set('path', 'root');

        const res = await fetch(uploadUrl.toString(), { method: 'POST', body: formData, headers: { 'Authorization': `Bearer ${import.meta.env.PUBLIC_UPLOAD_TOKEN}` } });
